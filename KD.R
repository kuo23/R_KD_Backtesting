...
#########################################
 ...
 analysis<-final %>% 
  mutate(ret=ifelse(stop_signal==1,stop_price/buyprice,sellprice/buyprice-1),                                          # 每次交易報酬率(不含交易成本)
         inPriceTc=buyprice*(1+0.0014),                                  # 含交易成本進場價
         outPriceTc=ifelse(stop_signal==1,stop_price*(1-0.0044),sellprice*(1-0.0044)),                                          # 每次交易報酬率(不含交易成本)
 ...
 original_retTc=(outPriceTc/inPriceTc-1)) %>% 
   select(ret,inPriceTc,outPriceTc,retTc,holdDays,stop_signal,original_holdDays,original_ret,original_inPriceTc,original_outPriceTc,original_retTc)

show1<-
tibble(
  holddays=mean(as.numeric(analysis$holdDays)),
  avgret=mean(as.numeric(analysis$ret)),
